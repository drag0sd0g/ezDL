...
import de.unidue.inf.is.ezdl.dlcore.message.content.RelatedTermsTell;
...
import de.unidue.inf.is.ezdl.dlservices.terminfo.services.dummy.DummyRelatedTermsService;



@StartedBy(RelatedTermsAsk.class)
public class RelatedTermsHandler extends AbstractRequestHandler {

    private TermInfoService relatedTermsService = new DummyRelatedTermsService();
    ...
    protected boolean work(Message message) {
        ...
        if (content instanceof RelatedTermsAsk) {
            handleRelatedTermsAsk(message, (RelatedTermsAsk) content);
        }
        else {
            handled = false;
        }
    ...
    private void handleRelatedTermsAsk(Message message, RelatedTermsAsk content) {
        ...
        send(message.tell(new RelatedTermsTell(terms)));
    }
}
