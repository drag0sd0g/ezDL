...
import de.unidue.inf.is.ezdl.dlservices.user.UserAgent;
import de.unidue.inf.is.ezdl.dlservices.user.store.UserStore;



abstract class AbstractUserAgentHandler extends AbstractRequestHandler {

    /**
     * Reference to the {@link UserStore};
     */
    private volatile UserStore userStore;


    /**
     * Returns a reference to the {@link UserStore} in the agent.
     * 
     * @return the {@link UserStore} reference
     */
    protected UserStore getStore() {
        if (userStore == null) {
            userStore = ((UserAgent) getAgent()).getUserStore();
        }
        return userStore;
    }
}
