 ...
 * @author Jens Kapitza
 */
public class YearExtractorTest extends AbstractTestBase {

    /**
     * The service we will test.
     */
    private ExtractorService service = new YearExtractor();
    ...
    public void extract() {
        ...
        Assert.assertEquals(2, authors.getTotal());
        Assert.assertEquals(1, authors.getMinimum());
        Assert.assertEquals(1, authors.getMaximum());
        for (Entry entry : authorList) {
            str.append(getHtml(entry.getKey(), entry.getValue()));
        }
        Assert.assertEquals("<span class='size6'>2009</span><span class='size6'>2010</span>", str.toString());
