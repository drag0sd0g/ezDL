...
@Reusable
@StartedBy(DeleteLibraryNotify.class)
public class DeleteLibraryHandler extends AbstractLibraryAgentHandler {
    ...
    private void handleDeleteLibrary(Message message, DeleteLibraryNotify n) {
        final int uid = getAgent().userIdForSessionId(n.getSessionId());
        try {
            getLibraryManager().removeDocument(n.getDocument(), uid, n.getReferenceSystem());
        }
        catch (Exception e) {
