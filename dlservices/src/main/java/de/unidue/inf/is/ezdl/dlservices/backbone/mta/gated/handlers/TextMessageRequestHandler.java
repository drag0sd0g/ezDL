...
@StartedBy(TextMessageNotify.class)
@Reusable
public class TextMessageRequestHandler extends AbstractRequestHandler {

    @Override
    protected boolean work(Message message) {
        MessageContent content = message.getContent();
        if (content instanceof TextMessageNotify) {
            handle((TextMessageNotify) content);
            return true;
        }
        return false;
    }
    ...
    private void handle(TextMessageNotify content) {
        AbstractGatedMTA mta = (AbstractGatedMTA) getAgent();
        TextMessageManager m = mta.getTextManager();
        m.handleNewsMessage(content);
    }
